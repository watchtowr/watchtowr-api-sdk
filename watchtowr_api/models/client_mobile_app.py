...
class ClientMobileApp(BaseModel):
    ...
    url: StrictStr = Field(...)
    s3path: StrictStr = Field(...)
    business_units: conlist(ClientBusinessUnit) = Field(..., alias="businessUnits")
    __properties = ["type", "source", "status", "created_at", "updated_at", "deleted_at", "id", "name", "publisher", "platform", "app_id", "url", "s3path", "businessUnits"]

    @validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('verified', 'Third Party', 'Unregistered', 'Incorrect Identification', 'pending', 'VerifiedOutOfScope', 'VerifiedReducedAttack', 'Tracked'):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ClientMobileApp:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ClientMobileApp:
        """Create an instance of ClientMobileApp from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ClientMobileApp.parse_obj(obj)

        _obj = ClientMobileApp.parse_obj({
            "type": obj.get("type"),
