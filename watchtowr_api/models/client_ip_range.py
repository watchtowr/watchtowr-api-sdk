...
class ClientIpRange(BaseModel):
    ...
    iprange: StrictStr = Field(...)
    asn: StrictStr = Field(...)
    desc: StrictStr = Field(...)
    country: StrictStr = Field(...)
    business_units: conlist(ClientBusinessUnit) = Field(..., alias="businessUnits")
    __properties = ["type", "source", "status", "created_at", "updated_at", "deleted_at", "id", "iprange", "asn", "desc", "country", "businessUnits"]

    @validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('verified', 'Third Party', 'Unregistered', 'Incorrect Identification', 'pending', 'VerifiedOutOfScope', 'VerifiedReducedAttack'):
            raise ValueError("must be one of enum values ('verified', 'Third Party', 'Unregistered', 'Incorrect Identification', 'pending', 'VerifiedOutOfScope', 'VerifiedReducedAttack')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ClientIpRange:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ClientIpRange:
        """Create an instance of ClientIpRange from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ClientIpRange.parse_obj(obj)

        _obj = ClientIpRange.parse_obj({
            "type": obj.get("type"),
